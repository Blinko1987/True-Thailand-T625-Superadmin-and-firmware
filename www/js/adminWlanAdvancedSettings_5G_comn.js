...
function initPage()
{
	...
	if(gDebug)
	{
		getDataByAjax("../fake/wlanAdvancedSettingsAll_5G", fillData);
	}else{
		XHR.get("wlanAdvancedSettingsAll_5G", null, fillData);
	}
}
...
function generateIndexSelect(ssidNumArray, ssidNumber, data)
{
	...
	for(; i <= number; i++)
	{
		if(ssidNumArray[i-1] != '0'){
			$("#wlsecurity_SSID").append("<option value='" + i + "'>" + data["ssid"+i].SSID + "</option>");
		}
	}
...
function saveApply()
{
	...
	showOrHideLoadingWindowFromIframe("show");
	if(gDebug){
		postDataByAjax("../fake/wlanAdvancedAll_5G", data);
	}else{
		...
		data.token = tokenstr;
		XHR.post("setWlanAdvancedCfgAll_5G", data, initPage);
	}
}
